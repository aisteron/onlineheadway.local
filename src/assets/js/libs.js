...
export async function load_mithril() {

	return new Promise(resolve => {

		let script = document.createElement('script')
		script.src = '/vendors/mithril.min.js'
		qs('.scripts-area').appendChild(script)
		script.onload = () => {
			let event = new CustomEvent("mithril_loaded", { detail: { loaded: true } })
...
export async function load_animate_css() {
	return new Promise(resolve => {
		let style = loadCSS('/vendors/animate.min.css')
		onloadCSS(style, () => {
			resolve('toast assets loaded')
		})
	})
...
export async function Fetch(action, data, path) {
	...
	let prot = process.env.NODE_ENV == 'development' ? 'http' : 'https'
	//let host = process.env.NODE_ENV == 'development' ? `${prot}://www.onlineheadway.com/` : ''
	let host = process.env.NODE_ENV == 'development' ? `${prot}://test.local` : ''
	let body = `action=${action}&data=${data}`
	//process.env.NODE_ENV == 'development' ? body += `&sessid=${SESSID}` : ''


	try {
		var response = await fetch(host + path, {
			method: 'POST',
